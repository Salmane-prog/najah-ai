...
def check_all_tests_final():
    """Vérifier tous les tests et identifier le problème"""
    ...
    try:
        ...
        # Vérifier si les tentatives référencent des tests inexistants
        if attempts:
            test_ids_in_attempts = set([attempt[1] for attempt in attempts])
            test_ids_in_tests = set([test[0] for test in tests])
            
            missing_tests = test_ids_in_attempts - test_ids_in_tests
            if missing_tests:
                print(f"  ❌ PROBLÈME IDENTIFIÉ: {len(missing_tests)} tentatives référencent des tests inexistants:")
                for missing_id in missing_tests:
                    print(f"     - Tentative pour le test {missing_id} mais ce test n'existe pas dans adaptive_tests")
            else:
                print("  ✅ Tous les tests référencés dans les tentatives existent")

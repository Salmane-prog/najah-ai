#!/usr/bin/env python3
"""
Script optimisé pour créer SEULEMENT les 3 tables manquantes
et ajouter des données de test pour l'utilisateur 30
"""
...
def create_only_missing_tables():
    """Créer seulement les 3 tables manquantes"""
    ...
    print(f"🚀 CRÉATION DES 3 TABLES MANQUANTES")
    print("=" * 50)
    ...
    try:
        # 1. Créer la table learning_paths
        ...
        # 2. Créer la table learning_path_steps
        ...
        # 3. Créer la table student_learning_paths
        ...
        conn.commit()
        print("\n🎉 3 tables manquantes créées avec succès!")
...
def create_test_data_for_user_30():
    ...
    try:
        ...
        print("2. 🗺️ Création de parcours d'apprentissage...")
...
def verify_final_state():
    """Vérifier l'état final après création"""
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('learning_paths', 'learning_path_steps', 'student_learning_paths')")
        ...
        print(f"   - Tables manquantes: {'✅ Créées' if len(created_tables) == 3 else '❌ Manquantes'}")
        
        if user and assessment_count > 0 and path_count > 0 and len(created_tables) == 3:
...
if __name__ == "__main__":
    print("🚀 CRÉATION OPTIMISÉE DES TABLES MANQUANTES")
    print("=" * 60)
    print("📋 Ce script crée SEULEMENT les 3 tables manquantes:")
    print("   1. learning_paths")
    print("   2. learning_path_steps") 
    print("   3. student_learning_paths")
    print("📋 Et utilise la table existante:")
    print("   ✅ student_assessment_results (pour les résultats)")
    print()
    
    create_only_missing_tables()
    create_test_user_30()
    create_test_data_for_user_30()
    verify_final_state()
    
    print("\n🎉 SCRIPT OPTIMISÉ TERMINÉ AVEC SUCCÈS!")
